...
namespace Com.PDMReader
{
    public class PdmFileReader : IPdmFileReader
    {
        ...
        public PdmFileReader(string pdm_file)  
        {  
            PdmFile = pdm_file;  
        }
        ...
        private PdmModels pdmModels;
        public PdmModels getPdmModels()
        {
            return pdmModels;
        }

        public TableInfo FindTableByTableName(string tableName)
        {
            return FindTableByCode(tableName);
        }

        private TableInfo FindTableByCode(string tableCode)
        {
            IList<TableInfo> tables = pdmModels.Tables;
            foreach (TableInfo mTable in tables)
            {
                if (mTable.Code == tableCode)
                    return mTable;
            }
            return null;
        }  
        private void InitData()
        {
            ...
            foreach (XmlNode xmlTables in xnTablesList)
            {
                foreach (XmlNode xnTable in xmlTables.ChildNodes)
                {
                    ...
                    {
                        pdmModels.Tables.Add(GetTable(xnTable));
                    }
                }
            }
            ...
            foreach (XmlNode xmlViews in xnViewsList)
            {
                ...
                {
                    pdmModels.Views.Add(GetView(xnView));
